...
export function MediaPlayer() {
  ...
  const handlePlayFullScreen = () => {
    if(videoRef.current){
      ...
      try{
        //tenta as diferentes versões de navegador
        const requestMethod = video.requestFullscreen || video.webkitRequestFullscreen || video.msRequestFullscreen || video.mozRequestFullScreen;
        if(requestMethod) {
          const promise = requestMethod.call(video);
